...
import { coopActivityBlockService } from "../../../services/activity/coopActivityBlockService";
...
type EditableBlock = CoopActivityBlock & {
  _isNew?: boolean;
  _isEditing?: boolean;
  _file?: File | null;
};

export function CoopActivityBlocksForm({ activity }: Props) {
  ...
  const loadBlocks = useCallback(async (): Promise<void> => {
    const data = await coopActivityBlockService.list(activity.id);
  ...
  const cancelEdit = (index: number): void => {
    setBlocks((prev) => {
      ...
      // Si era existente, desactivar edición
  ...
  const remove = async (block: EditableBlock, idx: number): Promise<void> => {
    askConfirm("¿Eliminar este bloque?", async () => {
      if (block.id) await coopActivityBlockService.remove(block.id);
  ...
  const saveOne = async (idx: number): Promise<void> => {
    ...
    setSaving(true);
    try {
      const payload: CoopActivityBlock = {
        title: b.title ?? "",
        body: b.body ?? "",
      ...
      let blockId = b.id;
      if (b.id) await coopActivityBlockService.update(b.id, payload);
      else {
        const created = await coopActivityBlockService.create(activity.id, payload);
        blockId = created.id;
      }

      if (b._file && blockId)
        await coopActivityBlockService.uploadImage(blockId, b._file);
