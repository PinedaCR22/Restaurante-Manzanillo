...
export default function Footer() {
  ...
  // clasificamos los contactos para mostrarlos ordenadamente
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 text-center">
        ...
        <div className="grid gap-8 sm:grid-cols-2 md:grid-cols-3 justify-items-center">
          ...
          <div className="max-w-xs flex flex-col items-center">
            ...
            {loading && (
              <p className="text-sm text-white/70">Cargando contactos…</p>
            )}

            {!loading && contacts.length === 0 && (
              <p className="text-sm text-white/70">Sin información disponible.</p>
            )}
            ...
            {!loading && contacts.length > 0 && (
              <>
                {/* Dirección */}
                {addresses.map((a) => (
                  <p key={a.id} className="text-sm text-white/90 flex items-center gap-2 mb-1">
                ...
                {/* Teléfonos */}
                {phones.map((p) => (
                  <p key={p.id} className="text-sm text-white/90 flex items-center gap-2 mb-1">
                    <FaPhone /> {p.value}
                  </p>
                ))}
                ...
                {/* Correos */}
                {emails.map((e) => (
                  <p key={e.id} className="text-sm text-white/90 flex items-center gap-2 mb-1">
                    <FaEnvelope /> {e.value}
                  </p>
                ))}
